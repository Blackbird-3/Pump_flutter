...
Future maybeCreateUser(User user) async {
  final userRecord = ComplaintsRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await ComplaintsRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createComplaintsRecordData(
    email: user.email ??
  ...
  currentUserDocument =
      ComplaintsRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createComplaintsRecordData(email: email));
}
